...
use mcp_set::source::{infer_name, parse_source};
use mcp_set::types::{Agent, McpServerConfig, PackageManager, Scope, Transport};
    ...
    match cli.command {
        Commands::Install {
            ...
            let parsed = if !extra_args.is_empty() {
                match parsed {
                    mcp_set::Source::Command { command, mut args } => {
                        args.extend(extra_args);
                        mcp_set::Source::Command { command, args }
                    }
                    other => other,
                }
            } else {
            ...
            let results = mcp_set::install(&config, &target_agents, scope);
        ...
        Commands::ListAgents => {
            for agent in Agent::ALL {
                let def = mcp_set::agent::agent_def(*agent);
        ...
        Commands::Detect { local } => {
            let detected = mcp_set::detect_agents(local);
